/* eslint-disable functional/no-loop-statements, max-lines-per-function -- Regression cases run sequential scenarios with explicit assertions. */
...
void test("uncertain publish failure reports the retained tarball before the write and never retries", async () => {
  ...
  // eslint-disable-next-line max-params -- Match the injectable exec adapter for failure testing.
